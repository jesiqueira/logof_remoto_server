...
# f.carraga_dados_servidor("lista_servidores.csv", 'login')
# f.logof()
# data = os.stat(os.path.join('csv', 'login_operador.csv')).st_atime
...
while opcao != 3:
    ...
    if opcao == 1:
        os.system('cls')
        f.carraga_dados_servidor('login')
    elif opcao == 2:
        os.system('cls')
        login = input('Informe o Login para realizar logof: ')
        f.logof(login.strip().lower(), 'login')
        os.system('cls')
    elif opcao == 3:
        os.system('cls')
        print('Você saiu')
